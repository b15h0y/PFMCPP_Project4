...
{
    if (rhs == 0.0f) std::cout << "warning, floating point division by zero returns 'inf' !" << std::endl;
    return lhs / rhs;
}
...
{
    if (!rhs) std::cout << "error, integer division by zero will crash the program! returning lhs" << std::endl;
